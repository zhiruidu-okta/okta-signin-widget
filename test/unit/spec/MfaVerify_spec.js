...
Expect.describe('MFA Verify', function() {
  ...
  const setupGoogleTOTPAutoPushTrue = _.partial(setup, Util.getAutoPushResponse(resAllFactors, true), {
  ...
  const setupOnPremTOTP = _.partial(setup, resAllFactorsOnPrem, { factorType: 'token', provider: 'DEL_OATH' });
  ...
  const setupEmail = _.partial(setup, resAllFactors, { factorType: 'email' });

  const setupOktaPush = _.partial(setup, resAllFactors, { factorType: 'push', provider: 'OKTA' });

  const setupOktaTOTP = _.partial(setup, resVerifyTOTPOnly, { factorType: 'token:software:totp' });
  ...
  const setupPassword = _.partial(setup, resPassword, { factorType: 'password' });
  ...
  const setupAllFactorsWithRouter = _.partial(setup, resAllFactors, null, { 'features.router': true });
  ...
  function setupU2F(options) {
    ...
    return setup(options.oneFactor ? resU2F : resAllFactors)
      .then(function(test) {
        const responses = [resChallengeU2F];
  ...
  Expect.describe('Factor types', function() {
    ...
    Expect.describe('Email', function() {
      ...
      itp('has a button to send the code', function() {
        return setupEmail().then(function(test) {
          const button = test.form.emailSendCode();
    ...
    Expect.describe('Okta Push', function() {
      ...
      Expect.describe('Push', function() {
        ...
        itp('calls authClient verifyFactor with correct args when autoPush is checked', function() {
          return setupOktaPush({ 'features.autoPush': true })
            .then(function(test) {
              $.ajax.calls.reset();
            ...
            .then(function() {
              ...
              Expect.isJsonPost($.ajax.calls.argsFor(0), {
                url:
                  'https://foo.com/api/v1/authn/factors/opfhw7v2OnxKpftO40g3/verify' +
            ...
            return setupOktaPush().then(function(test) {
              ...
              return tick(test).then(function() {
                expect(test.router.controller.model.stopListening).toHaveBeenCalledWith(
                  test.router.controller.model.appState,
                  'factorSwitched'
                );
              });
            });
