...
Expect.describe('EnrollSms', function() {
  ...
  const setupAndSendValidCode = _.partial(setupAndSendCode, resEnrollSuccess, 'US', '4151234567');

  const setupAndSendInvalidCode = _.partial(setupAndSendCode, resEnrollError, 'US', '415');
  ...
  Expect.describe('Enroll phone number', function() {
    ...
    itp('validation error if phone number field is blank', function() {
      return setupAndSendCode(resEnrollSuccess, 'US', '').then(function(test) {
        expect(test.form.hasErrors()).toBe(true);
      });
    });
    itp('enrolls with correct info when sendCode is clicked', function() {
      return setupAndSendCode(resEnrollSuccess, 'AQ', '12345678900').then(function() {
