...
const VALID_ID_TOKEN =
  'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJ2ZXIiOjEsIml' +
...
Expect.describe('LoginRouter', function() {
  ...
  it('throws a ConfigError if baseUrl is not passed as a widget param', function() {
    const fn = function() {
      setup({ authClient: new OktaAuth({ baseUrl: undefined }) });
    };

    expect(fn).toThrowError(
      'No url passed to constructor. Required usage: new OktaAuth({url: "https://sample.okta.com"})'
    );
  });
  itp(
  ...
  itp('has a success callback which correctly implements the setCookieAndRedirect function', function() {
    ...
    return setup({ globalSuccessFn: successSpy })
      ...
      .then(function() {
        const setCookieAndRedirect = successSpy.calls.mostRecent().args[0].session.setCookieAndRedirect;
  ...
  itp(
    ...
    function() {
      ...
      return setup({ stateToken: 'aStateToken', globalSuccessFn: successSpy })
        ...
        .then(function() {
          const targetUrl = successSpy.calls.mostRecent().args[0].stepUp.url;

          expect(targetUrl).toBe('http://foo.okta.com/login/step-up/redirect?stateToken=aStateToken');
          const finish = successSpy.calls.mostRecent().args[0].stepUp.finish;
  ...
  Expect.describe('OIDC - okta is the idp and oauth2 is enabled', function() {
    function expectAuthorizeUrl(url, options) {
      ...
      expectedUrl += '' + 'sessionToken=THE_SESSION_TOKEN&' + 'scope=openid%20email';
    ...
    itp('redirects instead of using an iframe if the responseType is "code"', function() {
      return setupOAuth2({ 'authParams.responseType': 'code' }).then(
        expectCodeRedirect({ responseMode: 'query', responseType: 'code' })
      );
    });
    ...
    itp('redirects if there are multiple responseTypes, and one is "code"', function() {
      return setupOAuth2({ 'authParams.responseType': ['id_token', 'code'] }).then(
        expectCodeRedirect({ responseMode: 'fragment', responseType: 'id_token%20code' })
      );
    });
    ...
    itp('calls the global error function if an idToken is not returned', function() {
      ...
      return setupOAuth2({ globalErrorFn: errorSpy })
        ...
        .then(function() {
          ...
          const err = errorSpy.calls.argsFor(0)[0];
  ...
  Expect.describe('Config: "assets"', function() {
    function expectBundles(baseUrl, login, country) {
      ...
      const loginCall = $.ajax.calls.argsFor(0)[0];
      const countryCall = $.ajax.calls.argsFor(1)[0];
    ...
    const expectDefaultCdn = _.partial(
      expectBundles,
    ...
    itp('can override bundles with both baseUrl and rewrite', function() {
      return setupLanguage({
        ...
        expectBundles('http://foo.com', '/labels/jsonp/login_ja.1.jsonp', '/labels/jsonp/country_ja.1.jsonp');
      });
    });
  });
