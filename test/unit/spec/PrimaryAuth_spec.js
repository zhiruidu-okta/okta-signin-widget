...
Expect.describe('PrimaryAuth', function() {
  ...
  Expect.describe('settings', function() {
    ...
    itp('uses default for username tooltip', function() {
      return setup().then(function(test) {
        const tip = test.form.usernameTooltipText();
    ...
    itp('uses default for password tooltip', function() {
      return setup().then(function(test) {
        const tip = test.form.passwordTooltipText();
  ...
  Expect.describe('customizing the tooltip', function() {
    itp('if the placeholder and tooltip differ,' + 'display both in the tooltip', function() {
    ...
    itp('if the placeholder and tooltip equal,' + 'display only one of them', function() {
    ...
    itp('if the placeholder is not defined,' + 'display only the tooltip', function() {
      spyOn(Okta, 'loc').and.callFake(function(key) {
  ...
  Expect.describe('elements', function() {
    ...
    itp('show username validation error when username field is dirty', function() {
      return setup()
        ...
        .then(function(test) {
          const msg = test.router.controller.model.validateField('username');
        ...
        .then(function(test) {
          const msg = test.router.controller.model.validateField('username').username;
    ...
    itp('show password validation error when password field is dirty', function() {
      return setup()
        ...
        .then(function(test) {
          const msg = test.router.controller.model.validateField('password');
        ...
        .then(function(test) {
          const msg = test.router.controller.model.validateField('password').password;
    ...
    itp('does not contain typing pattern header in primary auth request if feature is disabled', function() {
      return setup({ features: { trackTypingPattern: false } })
        ...
        .then(function() {
          ...
          expect($.ajax.calls.count()).toBe(1);
          const ajaxArgs = $.ajax.calls.argsFor(0);
  ...
  Expect.describe('Social Auth', function() {
    ...
    itp('opens a popup with the correct url when an idp button is clicked and asking for an accessToken', function() {
      return setupSocial({ 'authParams.responseType': 'token' }).then(function(test) {
        test.form.facebookButton().click();
        expect(window.open.calls.count()).toBe(1);
        expect(window.open).toHaveBeenCalledWith(
          'https://foo.com/oauth2/v1/authorize?' +
    ...
    itp(
      ...
      function() {
        return setupSocial({ 'authParams.responseType': ['id_token', 'token'] }).then(function(test) {
          test.form.facebookButton().click();
          expect(window.open.calls.count()).toBe(1);
          expect(window.open).toHaveBeenCalledWith(
            'https://foo.com/oauth2/v1/authorize?' +
